...
CFA-GDRO and EPH share a single per-sample loss $\ell^{\mathrm{EPH}}_i = \mathcal{L}_{\mathrm{EPH}}^{(t)}(f_i, y_i)$, which is fed into the sorted water-filling solver as the input to Eq.~\eqref{eq:method:cfa-gdro}. The total training objective is
\begin{equation}
...
\end{equation}
where $\overline{\ell^{\mathrm{EPH}}}$ is the mean evidential loss over the mini-batch and $\mathcal{L}_{\mathrm{CP\text{-}graph}}$ is the in-batch cross-pixel consistency loss defined in Eq.~\eqref{eq:method:cp-graph} below. The mean term keeps optimisation well-behaved when $\mathcal{L}_{\mathrm{CFA\text{-}GDRO}}^{\mathrm{EPH}}$ is sharp (small $\alpha$). Default weights are $\lambda_{\mathrm{rob}} = 0.5$, $\lambda_{\mathrm{graph}} = 0.1$, both tuned on validation only.
