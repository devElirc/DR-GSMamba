...
The center spectrum is treated as a one-dimensional ordered sequence. Each scalar band value is projected into a hidden feature space. The sequence is then processed by stacked state-space-style blocks. Each block contains layer normalization, gated projection, depthwise sequence mixing, and residual projection. This dependency-free implementation is used as a reproducible spectral SSM-style backbone; if an official selective-scan Mamba kernel is used in final experiments, the same interface can be retained while replacing this block.
