...
from losses import dr_gsmamba_loss
...
def run_epoch(model, loader, optimizer, device, cfg, train: bool):
    ...
    for batch in tqdm(loader, leave=False):
        ...
        with torch.set_grad_enabled(train):
            ...
            loss, _ = dr_gsmamba_loss(outputs, labels, cfg)
            if train:
...
def train_one_seed(cfg: dict, seed: int) -> dict:
    ...
    model = DRGSMamba(
        spectral_dim=cfg["dataset"]["pca_components"],
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        loss, y_true, y_pred = run_epoch(model, loaders["train"], optimizer, device, cfg, train=True)
