...
| Status snapshot | **Phase 0 done · Phase 1 done · Phase 2 done (2A–2E + D-11 polish) · Phase 3 implementation done (M3.1–M3.6) · D-14: Phase 5 M5.2 brought forward as a gate on Phase 6 (internal loss ablation + temperature scaling + GO/NO-GO verdict implemented, awaiting GPU run) · Phase 4 next** |
...
| M2C.6 | `models/cfa_gdro.py` assembles the full model and exposes `forward(spectrum, patch) -> (evidence, alpha, probs, vacuity, aleatoric, fused_feat)` |
...
- `D-13  2026-06-17 — Rename the main model from DR-GSMamba to CFA-GDRO throughout the codebase — the working paper title and contribution are CFA-GDRO + EPH; the old DR-GSMamba name was the superseded early idea (see D-01). Renamed: class DRGSMamba → CFAGDRO, module dr_gsmamba.py → cfa_gdro.py, config dr_gsmamba.yaml → cfa_gdro.yaml, defaults model key cfa_gdro. Historical decision-log entries that mention DR-GSMamba are left unchanged.`
- `D-14  2026-06-21 — Bring Phase-5 M5.2 internal loss ablation forward to gate Phase 3 / Phase 6 — client (Kass Kass, feedback.txt 2026-06-18) noted three signals in the Phase-2E smoke that block a confident "Reliable" claim: (i) worst-class accuracy 26.9% is too weak even though rare-class accuracy is 99.4% (the two are not the same class); (ii) ECE-15 = 0.473 is too high for a method that puts an evidential head in the story; (iii) external baselines are premature until CFA-GDRO is shown to actually beat CE / focal / sample-CVaR / Sagawa-Group-DRO on the same backbone + same data + same seed grid. Implemented: (a) loss-name selector in TrainConfig / Trainer so all five losses share backbone, EPH classifier head, data path, and seed policy; (b) post-hoc temperature scaling (Guo et al., 2017) in src/hsi_robust/eval/temperature_scaling.py, deterministically split (first calib_frac of test = fit, rest = report) so ECE_15_T is bit-reproducible; (c) scripts/run_internal_ablation.py runner that aggregates {OA, AA, kappa, worst_class, rare_class, ECE_15, ECE_15_T, T} over the seed grid and applies a structured GO/NO-GO verdict (CFA-GDRO must beat Sagawa on worst_class AND beat CVaR on rare_class AND stay within 2 pp of CE on OA — else block external baselines and tune α / γ / λ_evi or rework the title). All five losses are covered by parametrised unit tests and the verdict logic has dedicated tests.`
